...
class MinimagenDataset(torch.utils.data.Dataset):
    def __init__(self, hf_dataset, *, encoder_name: str, max_length: int,
        ...
        self.urls = hf_dataset[f"{split}"]['column0']
        self.captions = hf_dataset[f"{split}"]['column1']
...
def ConceptualCaptions(args, smalldata=False, testset=False):
    ...
    if smalldata:
        num = 16
        #vi = dset['validation']['image'][:num]
        #vc = dset['validation']['text'][:num]
        ti = dset['train']['column0'][:num]
        tc = dset['train']['column1'][:num]
